# Generate SpecDataDDT from MSP files:

# x <- ReadMspDirectory(directory = "./") # Be in working directory
# 
# write.csv(x, file = "SpecDataDDTWorking.csv", row.names = FALSE)
# write.table(x, file = "SpecDataDDT.csv", row.names = FALSE, sep = ";")

# Generate MSP text file:

# x <- MetaDataDDT[, c("CompoundName", "SpectrumFilename")]
...
LibraryReportDDT <- function(spectra = SpecData,
  ...
  grid.text("SpecLibDDT2015 Mass Spectral Library", y = 0.5, gp = gpar(cex = 1.25))
  ...
  grid.text("DDT Technical Mixture (500 ppm)", y = 0.5, gp = gpar(cex = 2))
